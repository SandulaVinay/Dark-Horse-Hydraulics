...
export const SolarCarSilhouette: React.FC<Props> = ({ activePartId }) => {
  ...
  useFrame((state) => {
    ...
    if (aero.current) {
      aero.current.position.y = lerp(5.8, 1.08, t);
    }
    if (root.current) {
  ...
  return (
    <group ref={root}>
      <group ref={shell}>
        {/* Low, rounded body volume — deliberately not box-shaped. */}
        <mesh scale={[1.48,.34,2.18]} position={[0,0,0]}>
          <sphereGeometry args={[1, 40, 24]} />
          <meshStandardMaterial {...bodyMat} />
        </mesh>
        {/* Front nose */}
        <mesh position={[0,-.01,1.96]} scale={[1.18,.27,.62]} rotation={[0,0,0]}>
          <sphereGeometry args={[1, 32, 20]} />
          <meshStandardMaterial {...bodyMat} />
        </mesh>
        {/* Rear taper */}
        <mesh position={[0,.02,-1.72]} scale={[1.28,.31,.74]}>
          <sphereGeometry args={[1, 32, 20]} />
          <meshStandardMaterial {...bodyMat} />
        </mesh>
        {/* Side rocker/fender lines */}
        {[-1,1].map((s) => (
          <mesh key={s} position={[s*1.34,.02,0]} scale={[.12,.18,1.78]}>
            <capsuleGeometry args={[.16,1.95,6,12]} />
            <meshStandardMaterial {...bodyMat} />
          </mesh>
        ))}
        {/* Front light strip */}
        <mesh position={[0,.16,2.48]} scale={[.72,.035,.035]}>
          <boxGeometry args={[1,.08,.08]} />
          <meshStandardMaterial color="#fff3d6" emissive="#fff3d6" emissiveIntensity={1.8} />
        </mesh>
        {/* Rear light strip */}
        <mesh position={[0,.15,-2.35]} scale={[.82,.035,.035]}>
          <boxGeometry args={[1,.08,.08]} />
          <meshStandardMaterial color="#b64b34" emissive="#8b2e22" emissiveIntensity={1.4} />
        </mesh>
      </group>

      <group ref={cabin}>
        {/* Aerodynamic cabin / canopy */}
        <mesh scale={[1.02,.56,1.05]} position={[0,0,.18]}>
          <sphereGeometry args={[1, 40, 24, 0, Math.PI*2, 0, Math.PI*.72]} />
          <meshPhysicalMaterial {...glassMat} />
        </mesh>
        {/* Windshield frame */}
        {[-1,1].map((s) => (
          <mesh key={s} position={[s*.86,.08,.52]} rotation={[0,s*.18,-s*.12]}>
            <capsuleGeometry args={[.035,.95,5,10]} />
            <meshStandardMaterial color="#aaa397" metalness={.88} roughness={.18} />
          </mesh>
        ))}
        {/* Roof solar panel */}
        <mesh position={[0,.44,.15]} rotation={[0,0,0]} scale={[.82,.045,.78]}>
          <boxGeometry args={[1.6,.09,1.5]} />
          <meshStandardMaterial color="#315a57" metalness={.4} roughness={.14} />
        </mesh>
      ...
      <group ref={aero}>
        {/* Splitter and rear aero wing give the silhouette a real vehicle stance. */}
        <mesh position={[0,0,2.54]} scale={[1.18,.05,.22]}>
          <boxGeometry args={[2.4,.1,.44]} />
          <meshStandardMaterial color="#34312b" metalness={.72} roughness={.25} />
        </mesh>
        <mesh position={[0,.48,-2.02]} scale={[1.05,.05,.12]}>
          <boxGeometry args={[2.15,.1,.24]} />
          <meshStandardMaterial color="#3a3731" metalness={.72} roughness={.22} />
        </mesh>
